...
const CrearNuevaOrdenDePago = async (req, res) => {
  try {
    ...
    const internCreateNewPayOrder = await pool.query(
      ...
      [idComercio, conceptoOrden, MontoOrdenTotal]
    );
    ...
    const Descripcion = conceptoOrden.concat(
      " Sr./Sra. ",
      nombreCompleto,
      "; ",
      formatedMerchantData.giro
    );
    ...
    res.status(200).json({
      internBdConult: internCreateNewPayOrder,
      externConsult: "hello world",
    });
  } catch (error) {
    console.log(error);
    console.log(error);
